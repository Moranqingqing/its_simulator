...
detector_params.add_induction_loop_detector(name='d1', lane_id='right0_0_0', position=-20, frequency=100)
# detector_params.add_induction_loop_detectors_to_intersection(name='d1', node_id='center0', positions=[-20], frequency=100)
...
def create_env(*_):
    ...
    for node_id in env.network.traffic_lights.get_properties().keys():
        tl = TrafficAgent(
            ...
            action_connector=ExtendChangePhaseConnector(node_id=node_id, phases=phases, tl_params=traffic_light_params, kernel=kernel),
