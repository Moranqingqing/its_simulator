...
class ConditionalVAE(tf.keras.Model):
    ...
    def __init__(self, action_size, obs_size, config_cvae, plot_model_path=None,
                 encoder=None, decoder=None):
        super(ConditionalVAE, self).__init__()
        self.latent_dim = config_cvae["z_size"]
        self.action_size = action_size
        self.obs_size = obs_size
        self.config_cvae = config_cvae

        if encoder is None:
            s = tf.keras.layers.Input(shape=[self.obs_size, ], name='s')

            s_ = tf.keras.layers.Input(shape=[self.obs_size, ], name='s_')
            a = tf.keras.layers.Input(shape=[self.action_size, ], name='a')

            if "do_not_use_s_" in config_cvae and not config_cvae["do_not_use_s_"]:
                inputs = [s, a]
            else:
                inputs = [s_, s, a]
            if config_cvae["type"] == HYPER_VAE:
                c = tf.keras.layers.Input(shape=[config_cvae["e_size"], ], name='c')
                inputs.append(c)

            self.encoder = tf.keras.Model(inputs=inputs, outputs=self.create_encoder(inputs))
        else:
            self.encoder = encoder

        if plot_model_path is not None:
            log.info("", "saving model architecture at {}".format(plot_model_path / "encoder.png"))
            tf.keras.utils.plot_model(self.encoder, show_shapes=True, dpi=64,
                                      to_file=str(plot_model_path / "encoder.png"))
        ...
        if decoder is None:
            ...
            if config_cvae["type"] == HYPER_VAE:
                c = tf.keras.layers.Input(shape=[config_cvae["e_size"], ], name='c')
                inputs.append(c)
            outputs = self.create_decoder(inputs)
            self.decoder = tf.keras.Model(inputs=inputs, outputs=outputs)
        else:
            self.decoder = decoder
        if plot_model_path is not None:
            log.info("", "saving model architecture at {}".format(plot_model_path / "decoder.png"))
            tf.keras.utils.plot_model(self.decoder, show_shapes=True, dpi=64,
                                      to_file=str(plot_model_path / "decoder.png"))

    @tf.function
    def sample(self, s, a, c=None):
        if "do_not_use_s_" in self.config_cvae and not self.config_cvae["do_not_use_s_"]:
            z = self.encode(s, a, c)
        else:
            z = tf.random.normal(shape=(len(s), self.latent_dim))
        x = self.decode(s, a, z, c)

        if self.config_cvae["reconstruction_loss"]["type"] == "gaussian_log_likelihood":
            params = self.config_cvae["reconstruction_loss"]["params"]
            if "decoder_output_std" in params and params["decoder_output_std"]:
                if "sample_decoder" in params and params["sample_decoder"]:
                    #  https://discuss.pytorch.org/t/multivariate-gaussian-variational-autoencoder-the-decoder-part/58235/5
                    mu_x, log_std_square_x = tf.split(x, num_or_size_splits=2, axis=1)
                    std_x = tf.math.sqrt(tf.math.exp(log_std_square_x))
                    gaussian = tfp.distributions.Normal(loc=mu_x, scale=std_x)
                    x = tf.squeeze(gaussian.sample(1))
                    if "sigmoid_output" in self.config_cvae and self.config_cvae["sigmoid_output"]:
                        x = tf.math.sigmoid(x)
                else:
                    mu_x, log_std_square_x = tf.split(x, num_or_size_splits=2, axis=1)
                    x = mu_x  # no need for sigmoid it, since it is already done by the layer

        return x

    def encode(self, s, a, s_, c=None):
        inputs = [s, a, s_]
        if c is not None:
            inputs.append(c)
    ...
    def decode(self, s, a, z, c=None):  # , apply_sigmoid=False):
        inputs = [s, a, z]
        if c is not None:
            inputs.append(c)
        fake_s_ = self.decoder(inputs)
        return fake_s_
...
class SamplerCVAE(ConditionalVAE):
    def __init__(self, decoder, config_cvae, plot_model_path=None):
        super(SamplerCVAE, self).__init__(None, None, config_cvae, plot_model_path, decoder=decoder, encoder="")


class DenseCVAE(ConditionalVAE):

    def __init__(self,
                 action_size, obs_size,
                 encoder_hiddens,
                 decoder_hiddens,
                 encoder_activation, decoder_activation, config_cvae, plot_model_path=None):
        self.encoder_hiddens = encoder_hiddens
        self.decoder_hiddens = decoder_hiddens
        self.encoder_activation = encoder_activation
        self.decoder_activation = decoder_activation
        super(DenseCVAE, self).__init__(action_size, obs_size, config_cvae, plot_model_path)
    ...
    def create_decoder(self, inputs):
        ...
        for layer in self.decoder_hiddens:
            x = tf.keras.layers.Dense(layer, activation=self.encoder_activation)(x)
        params_rec = self.config_cvae["reconstruction_loss"]["params"]
        type_rec = self.config_cvae["reconstruction_loss"]["type"]
