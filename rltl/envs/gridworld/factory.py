...
def grid0(wind, A=None, horizon=None, hole_absorbing=False):
    dim = (5, 5)
    dim_x, dim_y = dim
    if horizon is None:
        horizon = 3 * int(dim_x + dim_y)
    goals = [((4, 4, 5, 5), 1, 0, True)]
    # goals = []
    holes = [((2, 2, 3, 3), -1, 0, hole_absorbing), ((3, 2, 4, 3), -1, 0, hole_absorbing)]
    # holes = []
    ...
    start = (0.5, 0.5)
    if A == None:
        A = [(0., 1.), (0.414213, 0.414213), (1., 0.), (0., 0.5), (0.5, 0),
             (0.707106, 0.707106)]  # ,(-1.,0.),(0,-1.)]
    A_str = ["E", "SE", "S", "e", "s", "se"]  # ,"<-","^"]

    return lambda: EnvGridWorld(dim, (0.0, 0.0), cases, horizon,
...
def grid1(wind, A=None, horizon=None, hole_absorbing=False):
    dim = (11, 11)
    dim_x, dim_y = dim
    if horizon is None:
        horizon = 3 * int(dim_x + dim_y)
    goals = [((9, 9, 10, 10), 1, 0, True)]
    # goals = []
