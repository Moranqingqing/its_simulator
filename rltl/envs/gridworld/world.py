...
class World():
    def __init__(self, e, betas=None,do_weird_stuff_wtih_size=False):  # ,options={"pi": None, "actions": None, "id": "default_id"}):
        ...
        if do_weird_stuff_wtih_size:
            ...
            self.w_nb_blocks = np.ceil(np.sqrt(L * self.w_block / float(self.h_block)))
        if do_weird_stuff_wtih_size:
            self.dh = self.h_nb_blocks * self.h_block + self.h_txt_line * 2
        else:
            self.dh = self.h_nb_blocks * self.h_block
    ...
    def draw_trajectories(self, trajectories, rgba, line_width):
        for traj in trajectories:
            self.draw_trajectory(traj,rgba,line_width)
