...
class SlotFillingEnv(Env):
    ...
    def __init__(self, user_params={"cerr": -1, "cok": 1, "ser": 0.3, "cstd": 0.2, "proba_hangup": 0.3},
        ...
        self.observation_space = Box(low=-1.,
                                     high=max(max(self.max_turn, len(self.system_actions)), len(self.user_actions)),
                                     shape=(self.size_constraints + 2 * self.max_turn + 1,))
    ...
    def create_None_obs(self):
        recos_status = [None] * self.size_constraints
        str_sys_actions = [None] * self.max_turn
        str_user_actions = [None] * self.max_turn
        observation = {
            "recos_status": recos_status.copy(),
            "turn": self.turn,
            "str_sys_actions": str_sys_actions.copy(),
            "str_usr_actions": str_user_actions.copy()

        }
        return observation

    def flatten_obs(self, obs):
        recos_status = [-1. if a == None else a for a in obs["recos_status"]]
        sys_actions = [-1. if a == None else self.system_actions.index(a) for a in obs["str_sys_actions"]]
        usr_actions = [-1. if a == None else self.user_actions.index(a) for a in obs["str_usr_actions"]]
        res = np.array(recos_status + [obs["turn"]] + sys_actions + usr_actions)
        return res
    ...
    def step(self, action):
        ...
        if success:
            ...
            return self.flatten_obs(self.create_None_obs()), self.reward_if_sucess, True, {"c_": 0.}
        elif str_sys_action == "BYE":
            logger.info("[TURN {}] systeme ended dialogue premarturely !".format(self.turn))
            return self.flatten_obs(self.create_None_obs()), self.penalty_if_bye, True, {"c_": 0.}
        elif self.turn >= self.max_turn - 1:
            logger.info("[TURN {}] max size reached !".format(self.turn))
            return self.flatten_obs(self.create_None_obs()), self.penalty_if_max_turn, True, {"c_": 0.}
        elif str_user_action == "HANGUP":
            logger.info("[TURN {}] user hang up !".format(self.turn))
            return self.flatten_obs(self.create_None_obs()), self.penalty_if_hangup, True, {"c_": 1.}
        else:
