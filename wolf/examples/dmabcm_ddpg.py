        ...
        "params": {
            ...
            "reward_params": {
                ...
                "params": {
                    ...
                    "follow_ttc_flag": args.follow_ttc,
                }
            }
        }
    }
...
for eps in range(args.episodes):
    ...
    while True:
        ...
        for i in range(len(state)):
            ...
            next_state_= torch.Tensor([list(next_state[i])]).to(device)
            speed=0
            for j in range(len(state)):
                speed = speed+ state[j][0]
                # print('speed',speed)
            # average_speed=speed/len(state)
            # reward_=reward_+0.02*average_speed
...
if os.path.exists('/home/yifei/its_sow45/tmp.yaml'):
    ...
    exp_records[eff_type_key].append({
        'bcm_ddpg_7_states_multi_agent_no_constraint_no_speed': {
            'location': 'server',
            'notes': f'DDPG, single agent, state has 7 dim, f_eff type: {args.f_eff_type}',
