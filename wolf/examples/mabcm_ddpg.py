...
parser.add_argument("--save_dir", default=f"./tmp/bcm_ddpg_{unix_timestamp}_{time_string}/",
...
parser.add_argument("--speed-reward-weight", type=float, default=1., help="Weight for the speed limit reward component")
...
writer = SummaryWriter(f'runs/ddpg_{unix_timestamp}_{time_string}')
...
while timestep <= args.timesteps:
    ...
    while True:
        ...
        timestep += 1
        epoch_return += rewardavg

        mask = torch.Tensor([done]).to(device)
        reward = torch.Tensor([reward]).to(device)
        # next_state = torch.Tensor([next_state]).to(device)
        next_state_=torch.mean(next_state,dim=0)
        reward_=torch.mean(reward,dim=1)
        action_=torch.mean(action,dim=0)
        action_=torch.reshape(action_,(1,1))
        state_=torch.mean(state,dim=0)
        state_=torch.reshape(state_,(1,7))
        next_state_=torch.reshape(next_state_,(1,7))
        # print('reward shape',reward_.shape)
        # print('state shape',next_state_.shape)
        # print('done shape',mask.shape)
        # print('action',action_.shape)
        memory.push(state_, action_, mask, next_state_, reward_)
    ...
    #                 tmp_accels = []
    #                 for accel in accels:
    #                     tmp_accels.append(constrain_action(state, accel))
    #                 accels = tmp_accels
    ...
    #     # if np.mean(mean_test_rewards[-3:]) >= reward_threshold:
        ...
        if not os.path.exists(notes_path):
            with open(notes_path, 'w') as f:
                ...
                f"speed_reward weight: {args.speed_reward_weight}", file=f)
