...
parser.add_argument("--network", default="loop", choices=['loop', 'straight', 'merge', 'ring'])
...
additional_net_params = {"scaling": 1, "speed_limit": args.speed_limit, "length": 985, "width": 100, "rl_vehs": 2}

merge_additional_net_params = {
...
ring_additional_net_params = {
    # length of the ring road
    "length": 500,
    # number of lanes
    "lanes": 1,
    # speed limit for all edges
    "speed_limit": args.speed_limit if args.speed_limit else 30,
...
merge_net_params = NetParams(
    inflows=merge_inflow,
    additional_params=merge_additional_net_params
)

ring_net_params = NetParams(
    inflows=None,
    additional_params=ring_additional_net_params
)
...
if args.network == 'loop':
    network = CarFollowingNetwork(
        ...
        initial_config=InitialConfig(edges_distribution=['2']), # We only want the vehicle be place in the first edge
...
elif args.network == 'merge':
    network = MergeNetwork(
        name='merge',
        vehicles=vehicles,
        net_params=merge_net_params,
        initial_config=InitialConfig(edges_distribution=['inflow_highway']),
        traffic_lights=traffic_lights
    )
