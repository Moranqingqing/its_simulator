...
# === TEST 8   (Identical to Test 1 Bernoulli, but with a shorter horizon)
#              Test setting lane arrivals at environment initialization
...
lane_arrivals = []
lane_arrivals.append( ag.BernoulliArrivals(lane0, demand0) )
lane_arrivals.append( ag.BernoulliArrivals(lane1, demand1) )
lane_arrivals.append( ag.BernoulliArrivals(lane2, demand2) )

# Pool the lane arrivals and initialize the environment
...
env = WolfEnv.create_env(
    ...
    lane_arrival_sched=pooled_arrivals
)
...
# Run the simulation with resets
for _ in range(3):
    env.reset()
